layui.use(['table','layer'],function(){
    ...
    table.on('toolbar(cusDevPlans)', function(obj){
        switch(obj.event){
            case "add":
                ...
                break;
            case "success":
                updateSaleChanceDevResult($("input[name='id']").val(),2);//开发成功
                break;
            case "failed":
                updateSaleChanceDevResult($("input[name='id']").val(),3);//开发失败
                break;
        };
    });
    ...
    function updateSaleChanceDevResult(sid,devResult) {
        layer.confirm('确定执行当前操作吗？', {icon: 3, title: "计划项维护"}, function (index) {
            $.post(ctx+"/sale_chance/updateSaleChanceDevResult",
                {id:sid, devResult:devResult},function (data) {
                if(data.code==200){
                    layer.msg("操作成功！");
